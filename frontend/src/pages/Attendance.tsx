...
export default function Attendance() {
  ...
  /*
   * The URL is the single source of truth for the Attendance tab.
   * This allows notification deep-links such as:
   * /app/attendance?tab=exceptions
   * to open the correct section reliably.
   */
  ...
  const tab =
    isManager && requestedTab === "exceptions"
      ? "exceptions"
      : isManager && requestedTab === "team"
        ? "team"
        : "mine";

  const handleTabChange = (nextTab: string) => {
    if (nextTab === "exceptions" && isManager) {
      setSearchParams({ tab: "exceptions" });
      return;
    }

    if (nextTab === "team" && isManager) {
      setSearchParams({ tab: "team" });
      return;
    }

    // My Attendance is the default route.
    setSearchParams({});
  };
    ...
    <div className="px-6 pb-5">
      <div className="max-w-md">
        ...
        <p className="mt-1.5 text-[11px] text-ink-faint">
          {selectedEmployeeId
            ? "All AI attendance features will analyze the selected employee."
            : "All AI attendance features will analyze the overall attendance scope."}
        </p>
      </div>
    </div>
  </Card>
...
function TeamAttendanceExceptions() {
  ...
  const {
    ...
    queryFn: async () => {
      return AttendanceApi.teamRegularizationRequests(
        status,
      );
    },
  });
  ...
  const decisionMutation = useMutation({
    mutationFn: async ({
      ...
      if (action === "approve") {
        return AttendanceApi.approveRegularization(
          requestId,
          note,
        );
      }

      return AttendanceApi.rejectRegularization(
        requestId,
        note,
      );
    },
