  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <div className="rounded-2xl bg-gradient-to-br from-brand-50 to-gold-50 p-4 border border-line/60 shadow-sm max-w-xs">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          icon={Users}
          iconTone="brand"
          delta={{ value: `+${kpis.newHires30d} this month`, positive: true }}
        />
        <StatCard
          label={
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart
                  data={data.costTrend.map((c) => ({
                    ...c,
                    label: `${monthName(c.month).slice(0, 3)} '${String(c.year).slice(2)}`,
                  }))}
                >
                  <CartesianGrid vertical={false} stroke="#EFEEEB" />
                  <XAxis
