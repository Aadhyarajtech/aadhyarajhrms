...
export function AiCustomReportBuilderView({ filters, departments = [] }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-5 gap-2.5 print:hidden">
      ...
      {report && (
        <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-3 print:hidden">
      ...
      {/* Error */}
      {error && (
        <div className="rounded-xl border border-rose-200 bg-rose-50/70 p-3.5 text-rose-800 text-xs flex items-center gap-2.5 print:hidden">
      ...
      {/* Loading */}
      {isLoading && !report && (
        <div className="rounded-xl border border-slate-200 bg-white p-14 text-center space-y-2 print:hidden">
      ...
      {/* ─── 4. REPORT OUTPUT ────────────────────────────────────── */}
      {report && (
        <div ref={printableRef} className="space-y-4 printable-report print:space-y-3.5 print:w-full print:m-0 print:p-0">

          {/* Print-only Report Document Header */}
          <div className="hidden print:block mb-4 border-b-2 border-teal-700 pb-3">
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-[12pt] font-black uppercase tracking-wider text-teal-800">
                    AadhyaRaj HRMS
                  </span>
                  <span className="text-slate-300">•</span>
                  <span className="text-[9.5pt] font-bold text-slate-500 uppercase tracking-wide">
                    Executive Analytics Report
                  </span>
                </div>
                <h1 className="text-[22pt] font-extrabold text-slate-900 tracking-tight mt-0.5 leading-tight">
                  {report.title}
                </h1>
                <p className="text-[11pt] text-slate-600 font-medium">
                  {report.subtitle}
                </p>
              </div>
              <div className="text-right text-[9.5pt] text-slate-700 space-y-1 shrink-0 pl-6 border-l border-slate-200">
                <div><span className="font-bold text-slate-900">Dataset:</span> <span className="font-semibold text-teal-800">{report.dataset}</span></div>
                <div><span className="font-bold text-slate-900">Total Records:</span> {report.totalRecords}</div>
                <div><span className="font-bold text-slate-900">Generated:</span> {new Date(report.generatedAt || Date.now()).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}</div>
                <div><span className="font-bold text-slate-900">Engine:</span> {report.source === "llm" ? "Llama 3.3 70B AI" : "Deterministic Engine"}</div>
              </div>
            </div>
          </div>

          {/* ─── AI EXECUTIVE INTELLIGENCE BRIEF (Narrative + Findings + Action Plan) ─── */}
          <div className="rounded-2xl border border-teal-200/90 bg-gradient-to-br from-teal-50/50 via-white to-cyan-50/30 p-4 sm:p-5 shadow-xs transition-all print:border-slate-300 print:bg-white print:p-3.5 print-card">
            <div className="flex flex-wrap items-center justify-between gap-3 pb-2.5 border-b border-teal-100/80 print:border-slate-200">
              <div className="flex items-center gap-2.5">
                <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-teal-600 to-cyan-600 text-white flex items-center justify-center shadow-xs print:hidden">
                  <Sparkles size={16} />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-xs font-bold text-ink tracking-tight print:text-[13pt] print:font-extrabold print:text-slate-900">
                      AI Executive Intelligence Brief
                    </h3>
                    <span className="inline-flex items-center gap-1 text-[9px] font-semibold bg-teal-100/80 text-teal-800 px-2 py-0.5 rounded-full border border-teal-200 print:text-[9pt] print:bg-teal-50 print:border-teal-300">
                      {report.source === "llm" ? "✨ Llama 3.3 70B Engine" : "Deterministic Engine"}
                    </span>
                  </div>
                  <p className="text-[10px] text-ink-faint mt-0.5 print:text-[10pt] print:text-slate-500">
            ...
            <div className="mt-2.5">
              <div className="bg-white/80 p-3.5 rounded-xl border border-teal-100/80 text-xs text-slate-700 leading-relaxed font-medium shadow-2xs print:border-teal-200/80 print:bg-teal-50/30 print:p-3 print:text-[11pt] print:leading-relaxed print:text-slate-800">
                {report.executiveSummary}
              </div>
            </div>

            {/* Key Findings + Recommendations Grid */}
            <div className="mt-2.5 grid grid-cols-1 md:grid-cols-2 gap-3 print:grid-cols-2 print:gap-3">
              {/* Strategic Key Findings */}
              <div className="bg-white/95 p-3.5 rounded-xl border border-slate-200/80 space-y-2 shadow-2xs print:border-slate-300 print:p-3 print:bg-white print:space-y-1.5">
                <div className="text-[11px] font-bold text-ink flex items-center gap-1.5 print:text-[11.5pt] print:font-bold print:text-slate-900">
                  <TrendingUp size={13} className="text-teal-600 print:hidden" />
                  Key Strategic Observations
                </div>
                <ul className="space-y-1.5 print:space-y-1">
                  {report.keyFindings.map((finding, idx) => (
                    <li key={idx} className="text-[11px] text-ink-soft flex items-start gap-2 print:text-[10pt] print:text-slate-700">
                      <span className="h-1.5 w-1.5 rounded-full bg-teal-500 shrink-0 mt-1.5 print:mt-1 print:bg-teal-600" />
              ...
              <div className="bg-white/95 p-3.5 rounded-xl border border-slate-200/80 space-y-2 shadow-2xs print:border-slate-300 print:p-3 print:bg-white print:space-y-1.5">
                <div className="text-[11px] font-bold text-ink flex items-center gap-1.5 print:text-[11.5pt] print:font-bold print:text-slate-900">
                  <CheckCircle2 size={13} className="text-emerald-600 print:hidden" />
                  Recommended Action Plan
                </div>
                <div className="space-y-1.5 print:space-y-1">
                  {report.recommendations.map((rec, idx) => {
                    ...
                    return (
                      <div key={idx} className="text-[11px] p-2 rounded-lg bg-slate-50/80 border border-slate-100 print:border-slate-200 print:p-1.5 print:bg-slate-50/50">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <span className={`text-[9px] font-bold px-1.5 py-0.2 rounded border ${priorityColor} print:text-[8pt] print:px-1.5`}>
                            {rec.priority}
                          </span>
                          <span className="font-semibold text-ink truncate print:text-[10pt] print:text-slate-900">{rec.action}</span>
                        </div>
                        <p className="text-[10px] text-ink-faint italic print:text-[9pt] print:text-slate-500">{rec.impact}</p>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white overflow-hidden print:border-slate-300 print-card">
            <div className="flex divide-x divide-slate-100 print:divide-slate-200">
              {report.kpiCards.map((kpi) => {
                const statusDot =
                  ...
                  "bg-slate-300";
                return (
                  <div key={kpi.id} className="flex-1 p-4 text-center min-w-0 print:py-3 print:px-2">
                    <div className="flex items-center justify-center gap-1.5 mb-1 print:mb-0.5">
                      <div className={`h-1.5 w-1.5 rounded-full ${statusDot} print:h-2 print:w-2`} />
                      <span className="text-[10px] font-medium text-ink-faint truncate print:text-[9.5pt] print:font-semibold print:text-slate-600">{kpi.label}</span>
                    </div>
                    <div className="text-lg font-extrabold text-ink tracking-tight print:text-[17pt] print:font-black print:text-slate-900">{kpi.value}</div>
                    {kpi.subtext && (
                      <p className="text-[10px] text-ink-faint mt-0.5 print:text-[8.5pt] print:text-slate-500">{kpi.subtext}</p>
                    )}
                  </div>
                );
              })}
          ...
          {report.groupedSummary && report.groupedSummary.length > 0 && (
            <div className="space-y-2 print:space-y-1.5 print-card">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-ink flex items-center gap-1.5 print:text-[11.5pt] print:font-bold print:text-slate-900">
                  <LayoutGrid size={13} className="text-teal-600 print:hidden" />
                  Group: {activeGroupBy.charAt(0).toUpperCase() + activeGroupBy.slice(1)}
                  <span className="text-[10px] font-normal text-ink-faint ml-1 print:text-[9pt] print:text-slate-500">
                ...
                {drillFilter && (
                  <button
                    ...
                    className="text-[10px] text-teal-700 font-semibold hover:underline flex items-center gap-1 print:hidden"
                  >
                    <Filter size={10} /> Clear filter: "{drillFilter}"
                  </button>
                )}
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 print:grid-cols-6 print:gap-1.5">
                {report.groupedSummary.map((group) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-lg border text-center transition-all cursor-pointer group print:border-slate-300 print:bg-white print:p-1.5 print:cursor-default ${
                        isActive
                          ? "border-teal-400 bg-teal-50 ring-1 ring-teal-300"
                          : "border-slate-200 bg-white hover:border-teal-200 hover:bg-teal-50/30"
                      }`}
                    >
                      <div className="text-[10px] font-semibold text-ink truncate print:text-[8.5pt]" title={group.groupLabel}>
                        {group.groupLabel}
                      </div>
                      <div className="text-base font-extrabold text-teal-700 mt-0.5 print:text-[12.5pt]">{group.count}</div>
                      {/* Mini progress bar */}
                      <div className="mt-1 h-1 w-full rounded-full bg-slate-100 overflow-hidden print:bg-slate-200">
                        <div
                          className="h-full rounded-full bg-teal-500 transition-all"
                          style={{ width: `${Math.min(pct, 100)}%` }}
                        />
                      </div>
                      <div className="text-[9px] text-ink-faint mt-0.5 print:text-[8pt]">{pct}%</div>
                    </button>
                  );
                })}
              </div>
          ...
          {chartType !== "none" && report.chart && report.chart.data.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-white p-4 print:border-slate-300 print:p-3.5 print-card">
              <div className="flex items-center justify-between mb-3 print:mb-2">
                <h3 className="text-xs font-bold text-ink print:text-[12.5pt] print:font-bold print:text-slate-900">{report.chart.title}</h3>
                <span className="text-[10px] text-ink-faint print:text-[10pt] print:font-medium print:text-slate-500">{activeTab?.label} dataset</span>
              </div>
              <div className="h-52 w-full print:h-[300px] print:w-full print:block">
          ...
          {/* Data Table — with drill-down awareness, starts on Page 2 in Print */}
          <div className="rounded-xl border border-slate-200 bg-white overflow-hidden print:border-slate-300 print:overflow-visible print:break-before-page printable-table-section">
            <div className="px-4 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-2 bg-slate-50/50 print:px-3 print:py-2.5 print:border-slate-200 print:bg-slate-100/70">
              <div className="flex items-center gap-2">
                <Table2 size={13} className="text-teal-600 print:hidden" />
                <h3 className="text-xs font-bold text-ink print:text-[12pt] print:font-bold print:text-slate-900">Detailed Report Records</h3>
                <span className="text-[9px] font-semibold bg-teal-50 text-teal-700 px-1.5 py-0.5 rounded-md border border-teal-200 print:bg-white print:text-slate-700 print:border-slate-300 print:text-[8.5pt]">
                  {visibleColumnDefs.length} cols
                </span>
                {drillFilter && (
                  <span className="text-[9px] font-semibold bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded-md border border-amber-200 flex items-center gap-0.5 print:text-[8.5pt]">
                    <Filter size={9} className="print:hidden" /> {drillFilter}
                  </span>
                )}
              </div>
              <span className="text-[10px] text-ink-faint print:text-[9pt] print:font-medium print:text-slate-600">
                {filteredRows.length}{drillFilter ? ` of ${report.rows.length}` : ""} records
              </span>
            </div>

            <div className="overflow-x-auto print:overflow-visible">
              <table className="min-w-full text-left text-xs print:min-w-0 print:w-full print:table-auto">
                <thead>
                  <tr className="border-b border-slate-100 print:border-slate-300">
                    {visibleColumnDefs.map((col) => (
                      <th key={col.key} className="px-4 py-2.5 font-semibold text-ink-soft whitespace-nowrap text-[11px] bg-slate-50/30 print:whitespace-normal print:px-2.5 print:py-2 print:text-[8.5pt] print:font-bold print:bg-slate-100 print:text-slate-900 print:break-words">
                        {col.label}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filteredRows.length === 0 ? (
                    <tr>
                      <td colSpan={visibleColumnDefs.length || 1} className="p-8 text-center text-ink-faint text-xs print:p-6">
                        No records match the current scope.
                      </td>
                    </tr>
                  ) : (
                    filteredRows.map((row, idx) => (
                      <tr key={idx} className="border-b border-slate-50 last:border-0 hover:bg-teal-50/20 transition-colors print:border-slate-200 print:break-inside-avoid">
                        {visibleColumnDefs.map((col) => {
                          const val = row[col.key];
                          const isBadge = col.type === "badge";
                          const isCurrency = col.type === "currency";
                          return (
                            <td key={col.key} className="px-4 py-2.5 text-ink font-medium whitespace-nowrap text-[11px] print:whitespace-normal print:px-2.5 print:py-1.5 print:text-[8.5pt] print:break-words">
                              {isBadge ? (
                                <span className="inline-block px-2 py-0.5 rounded text-[9px] font-semibold bg-slate-100 text-slate-700 border border-slate-200 print:text-[7.5pt] print:px-1.5 print:py-0 print:bg-slate-100 print:border-slate-300">
                                  {String(val ?? "—")}
                                </span>
                              ) : isCurrency ? (
