...
payrollRouter.get("/salary-structure/:employeeId", isAdminOrFinance, async (req, res, next) => {
  try {
    res.json({ structure: (await repo.getSalaryStructure(req.params.employeeId)) ?? null });
  } catch (err) {
    next(err);
  }
});
...
payrollRouter.put("/salary-structure", isAdmin, validate(structureSchema), async (req, res, next) => {
  try {
...
payrollRouter.post("/runs/process", isAdminOrFinance, validate(processSchema), async (req, res, next) => {
  try {
    res.status(201).json({ run: await repo.processPayrollRun(req.body.month, req.body.year) });
  } catch (err) {
    next(err);
  }
});

payrollRouter.post("/runs/:id/mark-paid", isAdminOrFinance, async (req, res, next) => {
  try {
...
payrollRouter.get("/runs/:id/payslips", isAdminOrFinance, async (req, res, next) => {
  try {
...
payrollRouter.get("/payslips/mine", async (req, res, next) => {
  try {
    ...
    res.json({ payslips: await repo.listPayslipsForEmployee(req.user!.employeeId) });
  } catch (err) {
    next(err);
  }
});

payrollRouter.get("/payslips/employee/:employeeId", isAdminOrFinance, async (req, res, next) => {
  try {
    res.json({ payslips: await repo.listPayslipsForEmployee(req.params.employeeId) });
  } catch (err) {
    next(err);
  }
});
...
payrollRouter.get("/analytics/cost-trend", isAdminOrFinance, async (req, res, next) => {
  try {
