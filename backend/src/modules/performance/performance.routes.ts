...
export async function updateGoalProgress(id: string, progress: number) {
  ...
  if (typeof targetValue === "number" && targetValue > 0 && typeof currentValue === "number") {
    const nextCurrentValue = Math.min(
      targetValue,
      Math.max(0, Math.round((targetValue * safeProgress) / 100)),
    );
    const calculatedProgress = calculateGoalProgress(targetValue, nextCurrentValue) ?? 0;
    ...
    await Goal.updateOne(
      { _id: id },
      { $set: { progress: safeProgress, status: calculateGoalStatus(safeProgress) } },
    );
  }
...
export async function updateGoalCurrentValue(id: string, currentValue: number) {
  const goal = await Goal.findById(id).lean();
  if (!goal) return undefined;

  const targetValue = (goal as any).targetValue;
  const safeCurrentValue = Math.max(0, Number(currentValue));
  const progress = calculateGoalProgress(targetValue, safeCurrentValue);

  if (progress === null) {
    await Goal.updateOne({ _id: id }, { $set: { currentValue: safeCurrentValue } });
  } else {
    await Goal.updateOne(
      { _id: id },
      {
...
performanceRouter.post("/cycles", async (req, res, next) => {
  try {
...
performanceRouter.get("/reviews", async (req, res, next) => {
  try {
    const filters = {
      ...
      revieweeId: typeof req.query.revieweeId === "string" ? req.query.revieweeId : undefined,
      reviewerId: typeof req.query.reviewerId === "string" ? req.query.reviewerId : undefined,
    };
    res.json({ reviews: await listReviews(filters) });
  } catch (err) {
    next(err);
  }
});
...
performanceRouter.get("/reviews/mine", async (req, res, next) => {
  try {
    ...
    const reviews = await listReviews({ revieweeId: employeeId });
...
performanceRouter.post("/reviews", async (req, res, next) => {
  try {
...
performanceRouter.post("/reviews/:id/self", async (req, res, next) => {
  try {
    const review = await submitSelfReview(
      req.params.id,
      Number(req.body.selfRating),
      req.body.strengths ?? "",
...
performanceRouter.post("/reviews/:id/manager", async (req, res, next) => {
  try {
    const review = await submitManagerReview(
      req.params.id,
      Number(req.body.managerRating),
      req.body.managerComments ?? "",
      Number(req.body.managerTechnicalRating ?? req.body.managerRating),
      Number(req.body.managerDeliveryRating ?? req.body.managerRating),
      Number(req.body.managerBehaviorRating ?? req.body.managerRating),
    );
    res.json({ review });
  } catch (err) {
    next(err);
...
performanceRouter.get("/goals", async (req, res, next) => {
  try {
    ...
    res.json({ goals: await listGoals(employeeId) });
  } catch (err) {
    next(err);
  }
});

performanceRouter.post("/goals", async (req, res, next) => {
  try {
    const goal = await createGoal({
      ...req.body,
      employeeId: req.body.employeeId ?? (req as any).user?.employeeId,
    });
...
performanceRouter.get("/goals/:id", async (req, res, next) => {
  try {
    res.json({ goal: await getGoal(req.params.id) });
  } catch (err) {
    next(err);
  }
});

performanceRouter.patch("/goals/:id/progress", async (req, res, next) => {
  try {
    const goal = await updateGoalProgress(req.params.id, Number(req.body.progress));
    res.json({ goal });
  } catch (err) {
    next(err);
  }
...
performanceRouter.patch("/goals/:id/current-value", async (req, res, next) => {
  try {
    const goal = await updateGoalCurrentValue(
      req.params.id,
...
performanceRouter.get("/goals/:employeeId/trend", async (req, res, next) => {
  try {
    res.json({ trend: await getGoalTrend(req.params.employeeId) });
  } catch (err) {
    next(err);
  }
});

performanceRouter.get("/feedback/:reviewId/summary", async (req, res, next) => {
  try {
    res.json({ summary: await getFeedbackSummary(req.params.reviewId) });
  } catch (err) {
    next(err);
  }
});

performanceRouter.post("/feedback", async (req, res, next) => {
  try {
...
performanceRouter.get("/outcomes/:reviewId", async (req, res, next) => {
  try {
    res.json({ outcome: await getOutcome(req.params.reviewId) });
  } catch (err) {
    next(err);
  }
});

performanceRouter.put("/outcomes/:reviewId", async (req, res, next) => {
  try {
    const outcome = await upsertOutcome(req.params.reviewId, req.body);
    res.json({ outcome });
  } catch (err) {
    next(err);
  }
...
performanceRouter.get("/pips", async (req, res, next) => {
  try {
    ...
    res.json({ pips: await listPips(filters) });
  } catch (err) {
    next(err);
  }
});
...
performanceRouter.get("/pips/:id", async (req, res, next) => {
  try {
    res.json({ pip: await getPip(req.params.id) });
  } catch (err) {
    next(err);
  }
});

performanceRouter.post("/pips", async (req, res, next) => {
  try {
    const pip = await createPip(req.body);
...
performanceRouter.patch("/pips/:id/objectives", async (req, res, next) => {
  try {
    const pip = await updatePipObjectives(req.params.id, req.body.objectives ?? []);
    res.json({ pip });
  } catch (err) {
    next(err);
  }
...
performanceRouter.post("/pips/:id/check-ins", async (req, res, next) => {
  try {
    const pip = await addPipCheckIn(req.params.id, req.body);
    res.json({ pip });
  } catch (err) {
    next(err);
  }
...
performanceRouter.patch("/pips/:id/status", async (req, res, next) => {
  try {
...
performanceRouter.get("/analytics/rating-by-department", async (_req, res, next) => {
  try {
    res.json({ data: await getAverageRatingByDepartment() });
  } catch (err) {
    next(err);
  }
});