import { PerformanceCycle, PerformanceReview, Goal, Employee, Designation, Department } from "@/db/models";
...
export async function createCycle(input: { name: string; startDate: string; endDate: string }) {
...
export async function getActiveCycle() {
  const row = await PerformanceCycle.findOne({ isActive: true }).sort({ startDate: -1 }).lean();
  return toApiDoc(row);
}
...
async function enrichReviews(rows: any[]) {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      revieweeDesignation: reviewee ? desMap.get(reviewee.designationId)?.title ?? null : null,
      revieweeDepartment: reviewee ? deptMap.get(reviewee.departmentId)?.name ?? null : null,
...
export async function listReviews(filters: { cycleId?: string; revieweeId?: string; reviewerId?: string }) {
  ...
  const rows = await PerformanceReview.find(query).sort({ submittedAt: -1 }).lean();
...
export async function ensureReview(cycleId: string, revieweeId: string, reviewerId: string) {
  let row = await PerformanceReview.findOne({ cycleId, revieweeId }).lean();
  if (!row) {
    const doc = await PerformanceReview.create({ cycleId, revieweeId, reviewerId, status: "NOT_STARTED" });
    row = await PerformanceReview.findById(doc._id).lean();
  }
  return getReview((row as any)._id);
}

export async function submitSelfReview(id: string, selfRating: number, strengths: string, improvements: string) {
  await PerformanceReview.updateOne(
    { _id: id },
    { $set: { selfRating, strengths, improvements, status: "MANAGER_REVIEW" } }
  );
  return getReview(id);
}

export async function submitManagerReview(id: string, managerRating: number, managerComments: string) {
...
export async function createGoal(input: { employeeId: string; title: string; description?: string; dueDate: string }) {
...
export async function getAverageRatingByDepartment() {
  const reviews = await PerformanceReview.find({ finalRating: { $ne: null } }).lean();
