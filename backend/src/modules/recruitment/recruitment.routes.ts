...
const jobSchema = z.object({
  ...
  hiringMode: z.enum(["STANDARD", "WALK_IN", "CAMPUS"]).optional(),
  ...
  // Keep the frontend shortlisting configuration instead of silently
  // stripping it during Zod validation. This is consumed by
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      const job = await repo.updateJobPosting(req.params.id, req.body);
...
const rejectionSchema = z.object({
  reason: z.string().min(2, "Rejection reason is required.").max(1000),
});
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      const job = await repo.updateJobStatus(req.params.id, req.body.status);
...
recruitmentRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const candidates = await repo.listCandidates(jobPostingId);
...
recruitmentRouter.post(
  ...
  async (req, res, next) => {
    try {
      const candidate = await repo.createCandidate(req.body);
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      const candidate = await repo.updateCandidate(req.params.id, req.body);
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      if (
        req.body.resumeUrl === undefined &&
        req.body.resumeText === undefined
      ) {
        throw AppError.badRequest("Provide resumeUrl or resumeText.");
      }

      const candidate = await repo.setCandidateResume(req.params.id, req.body);
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      const candidate = await repo.rateCandidate(
        req.params.id,
        req.body.rating,
      );
...
recruitmentRouter.post(
  ...
  async (req, res, next) => {
    try {
      const candidate = await repo.screenCandidate(
        req.params.id,
        req.body.resumeText,
      );
...
recruitmentRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const interviews = await repo.listInterviews(candidateId);
...
recruitmentRouter.post(
  ...
  async (req, res, next) => {
    try {
      const interview = await repo.scheduleInterview(req.body);
...
recruitmentRouter.post(
  ...
  async (req, res, next) => {
    try {
      const interview = await repo.submitInterviewFeedback(
        req.params.id,
        req.body.feedback,
...
recruitmentRouter.post(
  ...
  async (req, res, next) => {
    try {
      const candidate = await repo.generateOffer(req.params.id, req.body);
...
const backgroundVerificationSchema = z.object({
  status: z.enum(["NOT_STARTED", "IN_PROGRESS", "VERIFIED", "FAILED"]),
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      const candidate = await repo.updateBackgroundVerification(
        req.params.id,
        req.body,
      );
...
recruitmentRouter.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const candidate = await repo.verifyPreboardingDocument(
        req.params.id,
        index,
      );
