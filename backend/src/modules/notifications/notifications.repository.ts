...
export async function notify(input: { userId: string; type: NotificationType; title: string; message: string; link?: string }) {
...
export async function listAnnouncements() {
  const rows = await Announcement.find({}).sort({ pinned: -1, createdAt: -1 }).lean();
  return rows.map(toApiDoc);
}

export async function createAnnouncement(input: { title: string; body: string; pinned?: boolean; audience?: string }) {
